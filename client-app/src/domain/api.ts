...
class Api {
    ...
    async getSpaces(teamId: number, archived: boolean = false) {
        return await this.get<Space[]>(`/team/${teamId}/space`, { archived: archived });
    }

    async getFolders(spaceId: number, archived: boolean = false) {
        return await this.get<Folder[]>(`/space/${spaceId}/folder`, { archived: archived });
    }

    async getLists(folderId: number, archived: boolean = false) {
        return await this.get<List[]>(`/folder/${folderId}/list`, { archived: archived });
    }
